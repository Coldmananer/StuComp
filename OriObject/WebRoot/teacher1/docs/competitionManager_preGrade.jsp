...
<html>
  ...
  <body class="app sidebar-mini rtl">
    ...
    <div class="container">
    	<p class="title">${sessionScope.competition.compName}竞赛预生成榜单</p>
			...
			<div class="tile">
				<div class="tile-body">
					<table class="table table-hover table-bordered" id="sampleTable">
						<thead>
							<tr>
								<th style="background-color:#009688;">排名</th>
								<th style="background-color:#009688;">队名</th>
								<th style="background-color:#009688;">成员</th>
								<th style="background-color:#009688;">成绩</th>
								<th style="background-color:#009688;">修改</th>
							</tr>
						</thead>
						<tbody>
							<s:iterator value="#request.pageBean.list" id="Complist">
								<tr>
									...
									<td><a href="showdetails?compListId=<s:property value="#Complist.compListId" />">修改</a></td>
							...
							<tr>
								<td colspan="5">
									<div>
										...
										<s:else>
											<a href="preGrade?CompId=<s:property value="#session.competition.CompId" />">首页</a>
            										&nbsp;&nbsp;&nbsp;
            									<a
												href="preGrade?CompId=<s:property value="#session.competition.CompId" />&page=<s:property value=" #request.pageBean.currentPage - 1"/>">上一页</a>
										</s:else>
										<s:if
											test="#request.pageBean.currentPage != #request.pageBean.totalPage && #request.pageBean.totalPage !=0 ">
											<a
												href="preGrade?CompId=<s:property value="#session.competition.CompId" />&page=<s:property value=" #request.pageBean.currentPage + 1"/>">下一页</a>
            										&nbsp;&nbsp;&nbsp;
            										<a
												href="preGrade?CompId=<s:property value="#session.competition.CompId" />&page=<s:property value=" #request.pageBean.totalPage"/>">尾页</a>
										</s:if>
										<s:else>
            										下一页&nbsp;&nbsp;&nbsp;尾页
        										</s:else>
					...
					<a href="showaddComplist">
   				 		<button style="border-style:none; width:120px; height:25px; color:#fff; background-color:#458B74;">添加成绩</button>
					</a>
					<!-- 
					<a href="showaddComplist">
                    	<input type="button" value="添加成绩" name="addScore"/>
                    </a> -->
                    <s:if test="#session.competition.compstatus.compStateId  == 4">
                    	<a href="resetcomplist">
   				 		<button style="border-style:none; width:120px; height:25px; color:#fff; background-color:#458B74;" onclick="return check()">重新生成榜单</button>
					</a>

                    </s:if>
                    <a href="compmanage?CompId=<s:property value="#session.competition.compId" />">
   				 		<button style="border-style:none; width:120px; height:25px; color:#fff; background-color:#458B74;">返回</button>
					</a>
                    <!--  
                    <a href="compmanage?CompId=<s:property value="#session.competition.compId" />">
